...
function iniciarApp() {
    try {
        ...
        let zs = []; 
        try { zs = JSON.parse(localStorage.getItem('zonas') || "[]"); if (!Array.isArray(zs) || zs.length === 0) zs = ["General"]; } catch(e) { zs = ["General"]; }
        ...
        if (r === 'Dueño' || r === 'Gerente') {
            ['btn-tab-registro','btn-tab-kardex','btn-tab-reportes','btn-tab-agotados','btn-tab-proveedores','btn-tab-orden','btnAgregarZona'].forEach(id => {
                const btnRef = document.getElementById(id); if(btnRef) btnRef.classList.remove('hidden');
            });
        ...
        // ESTE FUE EL CAMBIO 1: Entrar directamente al panel de ventas.
...
window.renderInventarioPantalla = () => {
    ...
    let lista = window.inventarioLocal.filter(p => {
        ...
        const coincideCat = (cat === "Todas" || p.categoria === cat); return coincideTxt && coincideCat;
    });
...
window.eliminarProducto = async (sku) => {
    ...
    try {
        ...
        if(pSnap.exists()) {
            ...
            window.mostrarNotificacion("🗑️ Producto eliminado correctamente"); window.cargarInventarioGeneral(); 
        }
...
window.agregarAlCarrito = (p) => {
    ...
    if (ex) { 
        ...
        else { return window.mostrarNotificacion("⚠️ Límite de stock alcanzado."); } 
    } else { 
...
// ESTE FUE EL CAMBIO 4: Permitir que "granel" lea decimales limpios y no se resetee a 1.
...
window.renderCarrito = () => {
    ...
    window.carrito.forEach((i, idx) => { 
        ...
        // ESTE FUE EL CAMBIO 4 (Visual): Modificar el "step" a 0.01 y poner el letrerito de L/Kg
        tb.innerHTML += `<tr class="border-b">
            ...
            <td class="py-4 text-center flex items-center justify-center gap-1">
                <input type="number" step="${i.es_granel ? '0.01' : '1'}" min="${i.es_granel ? '0.01' : '1'}" max="${i.stock}" value="${cant}" onchange="cambiarCantidadCarrito('${i.sku}', this.value)" class="w-16 md:w-20 p-2 border-2 rounded-xl text-center focus:ring-2 focus:ring-orange-500 font-bold">
                <span class="text-[10px] font-bold text-gray-500">${i.es_granel ? 'L/Kg' : 'Pz'}</span>
            </td>
...
window.procesarVentaCompleta = async () => {
    ...
    const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); const u = localStorage.getItem('currentUser');
    const ts = Date.now(); const f = new Date(ts).toLocaleString('es-MX'); 
    let tot = 0; 
...
// ======================================================================
// 8. MÓDULO ÓRDENES DE COMPRA (ESTILO VENTAS Y AUTOGENERACIÓN)
// ======================================================================

window.renderAgotados = () => {
    const tb = document.getElementById('tablaAgotadosBody');
    if(!tb) return;
    const filtroTipo = document.getElementById('alerta_filtro_tipo')?.value || 'todas';
    const filtroCat = document.getElementById('alerta_filtro_cat')?.value || 'Todas';
...
// ESTE FUE EL CAMBIO 2: Si el usuario da clic directamente en la pestaña en la computadora
...
window.generarOrdenCompra = () => {
    let faltantes = window.inventarioLocal.filter(p => parseFloat(p.stock) <= (parseFloat(p.min_stk) || 0));
    if(faltantes.length === 0) return alert("✅ Todo el inventario está por encima del mínimo. No se requiere orden de compra.");
    
    // Forzamos actualización por si había cambios
    window.ordenCompraActual = faltantes.map(p => {
        let sugerido = (parseFloat(p.max_stk) || 0) > 0 ? (parseFloat(p.max_stk) - parseFloat(p.stock)) : 1; 
        if (sugerido < 0) sugerido = 0;
        return { ...p, pedir: sugerido };
...
// ESTE FUE EL CAMBIO 3: Reemplazar botones + y - por una caja de texto idéntica a la de ventas
window.renderTabOrdenCompra = () => {
    ...
    window.ordenCompraActual.forEach((p, idx) => {
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border-2 border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex-1 w-full">
                ...
                <div class="flex gap-4 mt-1 text-sm text-gray-500 font-bold">
...
window.enviarOrdenWhatsApp = () => {
    ...
    filterPedir.forEach(p => {
        let subCosto = (parseFloat(p.costo) || 0) * p.pedir; totalCosto += subCosto;
...
window.guardarProveedor = async () => {
    ...
    try {
        ...
        document.querySelectorAll('.prov_dia').forEach(c => c.checked = false); window.cargarProveedores();
...
window.cargarProveedores = async () => {
    ...
    let h = ""; let opts = '<option value="">Seleccionar Proveedor...</option>'; 

    try {
        const sn = await getDocs(collection(db, `${e}_Proveedores`));
        sn.forEach(d => {
            const pr = d.data(); const diasStr = pr.dias.length > 0 ? pr.dias.join(', ') : 'Llamada previa';
        ...
        const p_prov = document.getElementById('p_prov'); const edit_prov = document.getElementById('edit_prov');
        if (p_prov) p_prov.innerHTML = opts; if (edit_prov) edit_prov.innerHTML = opts;
...
window.guardarProducto = async () => {
    const sku = document.getElementById('p_sku').value.trim(); const nom = document.getElementById('p_nom').value.trim(); const cat = document.getElementById('p_cat').value.trim() || "General";
    const prov = document.getElementById('p_prov')?.value || ""; const cos = parseFloat(document.getElementById('p_cos').value) || 0; const pre = parseFloat(document.getElementById('p_pre').value) || 0; 
    const pMay = parseFloat(document.getElementById('p_mayoreo').value) || 0; const cMay = parseFloat(document.getElementById('p_cant_mayoreo').value) || 0; const stk = parseFloat(document.getElementById('p_stk').value) || 0; 
    const minS = parseFloat(document.getElementById('p_min').value) || 0; const maxS = parseFloat(document.getElementById('p_max').value) || 0;
    const isGranel = document.getElementById('p_granel').checked; const cad = document.getElementById('p_caducidad').value || null;
    ...
    try {
        const r = `${e}_Inventario_${z}`; const d = await getDoc(doc(db, r, sku)); let sF = stk; if (d.exists()) sF += d.data().stock;
        ...
        if(document.getElementById('p_prov')) document.getElementById('p_prov').value = ''; document.getElementById('p_granel').checked = false; window.cargarInventarioGeneral();
...
window.abrirModalEdicion = (sku) => { 
    ...
    document.getElementById('edit_sku').value = p.sku; document.getElementById('edit_nom').value = p.nombre; document.getElementById('edit_cat').value = p.categoria || "General";
    if(document.getElementById('edit_prov')) document.getElementById('edit_prov').value = p.proveedor || "";
    document.getElementById('edit_cos').value = p.costo||0; document.getElementById('edit_pre').value = p.precio||0; document.getElementById('edit_promo').value = p.precio_promo || 0; 
    document.getElementById('edit_mayoreo').value = p.precio_mayoreo || 0; document.getElementById('edit_cant_mayoreo').value = p.cant_mayoreo || 0; document.getElementById('edit_stk').value = p.stock||0; 
    document.getElementById('edit_min').value = p.min_stk || 0; document.getElementById('edit_max').value = p.max_stk || 0; document.getElementById('edit_granel').checked = p.es_granel || false; 
    document.getElementById('edit_caducidad').value = p.caducidad || ""; document.getElementById('modalEdicion').classList.remove('hidden'); 
};

window.cerrarModalEdicion = () => { document.getElementById('modalEdicion').classList.add('hidden'); };

window.guardarEdicionProducto = async () => {
    const sku = document.getElementById('edit_sku').value; const nom = document.getElementById('edit_nom').value.trim(); const cat = document.getElementById('edit_cat').value.trim() || "General"; 
    const prov = document.getElementById('edit_prov')?.value || ""; const cos = parseFloat(document.getElementById('edit_cos').value) || 0; const pre = parseFloat(document.getElementById('edit_pre').value) || 0; 
    const promo = parseFloat(document.getElementById('edit_promo').value) || 0; const pMay = parseFloat(document.getElementById('edit_mayoreo').value) || 0; const cMay = parseFloat(document.getElementById('edit_cant_mayoreo').value) || 0; 
    const stk = parseFloat(document.getElementById('edit_stk').value) || 0; const minS = parseFloat(document.getElementById('edit_min').value) || 0; const maxS = parseFloat(document.getElementById('edit_max').value) || 0;
    const isGranel = document.getElementById('edit_granel').checked; const cad = document.getElementById('edit_caducidad').value || null;
    ...
    try {
        ...
        window.mostrarNotificacion("✅ Producto actualizado"); window.cerrarModalEdicion(); window.cargarInventarioGeneral();
    } catch(err) { alert("Error al editar."); }
};

window.procesarArchivoMasivo = function() {
    const i = document.getElementById('archivoInventario'); const s = document.getElementById('statusCargaMasiva'); const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); const u = localStorage.getItem('currentUser');
    if (!i.files || i.files.length === 0) return; const a = i.files[0]; const l = new FileReader();
    ...
    l.onload = async function(ev) {
        ...
        try {
            let batch = writeBatch(db); let opCount = 0; const commitBatch = async () => { if(opCount > 0) { await batch.commit(); batch = writeBatch(db); opCount = 0; } };
            ...
            if (a.name.endsWith('.xml')) {
                ...
                const ln = c.split('\n'); for (let li of ln) { const col = li.trim().split(','); if (col.length >= 5 && col[0].trim()) { await procesarItem(col[0].trim(), col[1].trim(), parseFloat(col[4])||0, parseFloat(col[2])||0, parseFloat(col[3])||0); pr++; } } 
            }
            await commitBatch(); s.className = "mt-4 p-4 bg-green-50 text-green-700 rounded-lg font-bold"; s.innerText = `✅ Carga Exitosa! ${pr} productos cargados.`; window.cargarInventarioGeneral();
        } catch (er) { s.className = "mt-4 p-4 bg-red-50 text-red-700 rounded-lg"; s.innerText = "❌ Error en archivo."; }
    }; l.readAsText(a);
};

window.vaciarInventario = async () => { 
    const z = document.getElementById('zonaSelect').value; const p = prompt(`Precaución. Escribe VACIAR para borrar toda la zona ${z}:`); if(p !== 'VACIAR') return; 
    try { 
        const sn = await getDocs(collection(db, `${localStorage.getItem('empresaId')}_Inventario_${z}`)); sn.forEach(async (d) => { await deleteDoc(doc(db, `${localStorage.getItem('empresaId')}_Inventario_${z}`, d.id)); }); 
        alert("Inventario borrado con éxito."); window.cargarInventarioGeneral(); 
...
window.crearUsuarioSecundario = async () => { 
    ...
    try { 
        ...
        alert(`✅ Usuario creado con éxito.`); ['new_user', 'new_email', 'new_pass'].forEach(id => document.getElementById(id).value = ''); window.cargarUsuarios(); 
...
window.cargarUsuarios = async () => { 
    ...
    try { 
        ...
        sn.forEach(d => { const u = d.data(); h += `<tr class="border-b hover:bg-blue-50 transition"><td class="p-3 font-bold">${u.usuario || d.id}</td><td class="p-3 font-bold ${u.rol==='Dueño'?'text-blue-600':'text-green-600'}">${u.rol}</td><td class="p-3 text-gray-500">${u.zonas.join(', ')}</td></tr>`; }); 
...
window.agregarNuevaZona = async () => { 
    ...
    if(nZ) { 
        const u = localStorage.getItem('currentUser'); const z = JSON.parse(localStorage.getItem('zonas') || "[]"); 
        if(!z.includes(nZ)) { z.push(nZ); await setDoc(doc(db, "SaaS_Usuarios", u), { zonas: z }, { merge: true }); localStorage.setItem('zonas', JSON.stringify(z)); alert("Zona agregada correctamente. La página se actualizará."); location.reload(); } else { alert("Esa zona ya existe en tu registro."); }
    } 
};
...
window.abrirModalRegistroRapido = () => {
    const busquedaActual = document.getElementById('venta_busqueda').value; document.getElementById('rr_sku').value = busquedaActual.trim(); document.getElementById('rr_nombre').value = ''; document.getElementById('rr_costo').value = ''; document.getElementById('rr_precio').value = ''; document.getElementById('modal-registro-rapido').classList.remove('hidden');
...
window.guardarRegistroRapido = async () => {
    ...
    try {
        ...
        window.inventarioLocal.push(nuevoProducto); window.cerrarModalRegistroRapido(); document.getElementById('venta_busqueda').value = '';
        const productoParaCarrito = { ...nuevoProducto, stock: 9999 }; window.agregarAlCarrito(productoParaCarrito); window.mostrarNotificacion('✅ Producto express guardado y listo para cobro.');
