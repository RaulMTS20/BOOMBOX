...
window.registrarNegocio = async () => { 
    ...
    try { 
        ...
        alert("Negocio creado. Inicia sesión."); window.toggleAuth(); 
...
window.iniciarSesion = async () => { 
    ...
    if(!u || !p) return alert("Faltan datos"); btn.innerText = "Conectando..."; btn.disabled = true;
    try {
        ...
        if (userDoc.exists()) { 
            const d = userDoc.data(); localStorage.setItem('currentUser', u); localStorage.setItem('empresaId', d.empresaId); 
            localStorage.setItem('zonas', JSON.stringify(d.zonas || [])); localStorage.setItem('userRol', d.rol || "Vendedor"); 
            iniciarApp(); 
        }
    } catch (er) { alert("Contraseña incorrecta o error de conexión."); } finally { btn.innerText = "Entrar"; btn.disabled = false; }
};

window.recuperarPassword = async () => { 
    const u = prompt("Ingresa tu Usuario:"); if(!u) return; 
    try { 
        const dirDoc = await getDoc(doc(db, "SaaS_Directorio", u)); if(!dirDoc.exists()) return alert("No existe el usuario."); 
        await sendPasswordResetEmail(auth, dirDoc.data().email); alert(`Enlace enviado al correo registrado.`); 
...
function iniciarApp() {
    try {
        ...
        const e = localStorage.getItem('empresaId') || "Negocio"; const u = localStorage.getItem('currentUser') || "Usuario"; const r = localStorage.getItem('userRol') || "Vendedor"; 
        let zs = []; try { zs = JSON.parse(localStorage.getItem('zonas') || "[]"); if (!Array.isArray(zs) || zs.length === 0) zs = ["General"]; } catch(e) { zs = ["General"]; }
        
        document.getElementById('displayEmpresa').innerText = e; document.getElementById('displayUser').innerText = `${u} (${r})`;
        const sZ = document.getElementById('zonaSelect'); sZ.innerHTML = ""; zs.forEach(z => sZ.add(new Option(z, z)));

        if (r === 'Dueño' || r === 'Gerente') {
            ['btn-tab-registro','btn-tab-kardex','btn-tab-reportes','btn-tab-agotados','btn-tab-proveedores','btn-tab-orden','btnAgregarZona'].forEach(id => { const btnRef = document.getElementById(id); if(btnRef) btnRef.classList.remove('hidden'); });
            document.querySelectorAll('.costo-col').forEach(el => el.classList.remove('hidden')); document.querySelectorAll('.accion-col').forEach(el => el.classList.remove('hidden'));
        } else { 
            document.querySelectorAll('.costo-col').forEach(el => el.classList.add('hidden')); document.querySelectorAll('.accion-col').forEach(el => el.classList.add('hidden')); 
        }
...
window.iniciarCamara = (m) => { 
    document.getElementById(`reader-${m}`).classList.remove('hidden'); document.getElementById(`btn-iniciar-camara-${m}`).classList.add('hidden'); document.getElementById(`btn-detener-camara-${m}`).classList.remove('hidden');
    ...
    scanners[m].start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, (decodedText) => {
        ...
        if(m === 'ventas') { 
            ...
            if(p) { window.agregarAlCarrito(p); } else { document.getElementById('venta_busqueda').value = decodedText; window.abrirModalRegistroRapido(); window.mostrarNotificacion("⚠️ Nuevo producto detectado."); }
        } else { document.getElementById('p_sku').value = decodedText; }
    }, (e) => { }).catch(err => { alert("Cámara no soportada."); });
};

window.detenerCamara = (m) => { 
    if(scanners[m]) { scanners[m].stop().then(() => { document.getElementById(`reader-${m}`).classList.add('hidden'); document.getElementById(`btn-iniciar-camara-${m}`).classList.remove('hidden'); document.getElementById(`btn-detener-camara-${m}`).classList.add('hidden'); }).catch(e=>console.log(e)); }
};

function poblarFiltros() {
    let htmlCat = `<option value="Todas">Todas las categorías</option>`; window.categoriasUnicas.forEach(c => { if(c) htmlCat += `<option value="${c}">${c}</option>`; });
    const els = ['filtro_inv_cat', 'alerta_filtro_cat', 'rep_cat']; els.forEach(id => { const el = document.getElementById(id); if(el) el.innerHTML = htmlCat; });
}

window.cargarInventarioGeneral = async () => {
    const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); if(!z || !e) return;
    ...
    try {
        ...
        sn.forEach((d) => { const p = d.data(); window.inventarioLocal.push(p); if(p.categoria) window.categoriasUnicas.add(p.categoria); });
        poblarFiltros(); window.renderInventarioPantalla(); if(!document.getElementById('tab-agotados').classList.contains('hidden')) window.renderAgotados();
...
window.renderInventarioPantalla = () => {
    ...
    const txt = document.getElementById('filtro_inv_txt').value.toLowerCase(); const cat = document.getElementById('filtro_inv_cat').value; const ord = document.getElementById('filtro_inv_ord').value;
    ...
    if(ord === 'nombre') lista.sort((a,b) => a.nombre.localeCompare(b.nombre)); if(ord === 'stock_asc') lista.sort((a,b) => a.stock - b.stock); if(ord === 'stock_desc') lista.sort((a,b) => b.stock - a.stock);
    let h = "";
    lista.forEach(p => {
        ...
        const ca = isManager ? `<td class="p-3 text-center accion-col md:min-w-[200px]">
            <button onclick="abrirModalEdicion('${p.sku}')" class="text-blue-600 bg-blue-50 px-3 py-2 rounded font-bold hover:bg-blue-100 transition mb-1 md:mb-0 md:mr-2 text-lg">✏️</button>
            <button onclick="eliminarProducto('${p.sku}')" class="text-red-600 bg-red-50 px-3 py-2 rounded font-bold hover:bg-red-100 transition text-lg">🗑️</button></td>` : `<td class="p-3 text-left accion-col hidden"></td>`;
        ...
        if(p.caducidad) { const diff = Math.floor((new Date(p.caducidad) - Date.now()) / 86400000); if(diff <= 30) badges += `<span class="bg-red-100 text-red-700 text-[10px] px-2 py-1 rounded block w-fit font-bold">Vence en ${diff}d</span>`; }
...
window.eliminarProducto = async (sku) => {
    if(!confirm(`⚠️ ¿Estás seguro de ELIMINAR el producto ${sku}?`)) return;
    ...
    try {
        ...
        if(pSnap.exists()) { await deleteDoc(pRef); await addDoc(collection(db, `${e}_Historial_Ingresos`), { sku, nombre: pSnap.data().nombre, cantidad: pSnap.data().stock, zona: z, usuario: u, fechaRegistro: new Date().toLocaleString('es-MX'), timestamp: Date.now(), tipoMovimiento: "ELIMINACIÓN" }); window.mostrarNotificacion("🗑️ Producto eliminado correctamente"); window.cargarInventarioGeneral(); }
    } catch(er) { alert("Ocurrió un error."); }
};

window.filtrarProductosVenta = () => {
    const i = document.getElementById('venta_busqueda').value.toLowerCase(); const c = document.getElementById('sugerencias_venta'); c.innerHTML = '';
    ...
    if (r.length > 0) { 
        ...
        r.forEach(p => { 
            ...
            const d = document.createElement('div'); d.className = "p-4 hover:bg-orange-50 cursor-pointer border-b flex justify-between items-center"; 
            ...
            d.onclick = () => { window.agregarAlCarrito(p); document.getElementById('venta_busqueda').value = ''; c.classList.add('hidden'); }; c.appendChild(d); 
        });
...
window.agregarAlCarrito = (p) => {
    ...
    if (ex) { if(ex.cantidad < p.stock) { ex.cantidad++; ex.precioVentaReal = window.calcularPrecioExacto(ex, ex.cantidad); } else { return window.mostrarNotificacion("⚠️ Límite de stock alcanzado."); } } 
    else { let n = { ...p, cantidad: 1 }; n.precioVentaReal = window.calcularPrecioExacto(n, 1); window.carrito.push(n); }
    window.mostrarNotificacion(`✅ Agregado al carrito`); window.renderCarrito();
};

window.cambiarCantidadCarrito = (sku, val) => { 
    let cant = parseFloat(val); if (isNaN(cant)) return; 
    const item = window.carrito.find(p => p.sku === sku); 
    if(item) { 
        if(!item.es_granel && cant % 1 !== 0) { window.mostrarNotificacion("⚠️ Este producto no se vende a granel."); cant = Math.floor(cant); }
        if(cant > item.stock) { window.mostrarNotificacion(`⚠️ Solo hay ${item.stock} en stock.`); cant = item.stock; }
        if(cant <= 0) cant = item.es_granel ? 0.01 : 1;
        item.cantidad = cant; item.precioVentaReal = window.calcularPrecioExacto(item, cant); window.renderCarrito(); 
    } 
};
...
window.renderCarrito = () => {
    ...
    if (window.carrito.length === 0) { tb.innerHTML = '<tr><td colspan="4" class="text-center text-gray-400 py-8">El carrito está vacío</td></tr>'; document.getElementById('gran-total').innerText = "$0.00"; return; }
    
    window.carrito.forEach((i, idx) => { 
        const sub = i.precioVentaReal * i.cantidad; t += sub; 
        const cant = i.cantidad % 1 !== 0 ? parseFloat(i.cantidad).toFixed(3) : i.cantidad;
        ...
        tb.innerHTML += `<tr class="border-b"><td class="py-4 font-bold text-gray-700 text-xs">${i.nombre}${badge}</td><td class="py-4 text-center flex items-center justify-center gap-1"><input type="number" step="${i.es_granel ? '0.01' : '1'}" min="${i.es_granel ? '0.01' : '1'}" max="${i.stock}" value="${cant}" onchange="cambiarCantidadCarrito('${i.sku}', this.value)" class="w-16 md:w-20 p-2 border-2 rounded-xl text-center focus:ring-2 focus:ring-orange-500 font-bold"><span class="text-[10px] font-bold text-gray-500">${i.es_granel ? 'L/Kg' : 'Pz'}</span></td><td class="py-4 text-right text-green-600 font-bold">$${sub.toFixed(2)}</td><td class="py-4 text-right"><button onclick="quitarDelCarrito(${idx})" class="text-red-500 hover:scale-110 transition-transform">🗑️</button></td></tr>`; 
    }); 
...
window.procesarVentaCompleta = async () => {
    ...
    const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); const u = localStorage.getItem('currentUser'); const ts = Date.now(); const f = new Date(ts).toLocaleString('es-MX'); let tot = 0; 
    try {
    ...
    } catch(er) { alert("Ocurrió un error."); btn.disabled = false; btn.innerText = "✅ Cobrar"; } 
};

// 8. MÓDULO ÓRDENES DE COMPRA
window.renderAgotados = () => {
    const tb = document.getElementById('tablaAgotadosBody'); if(!tb) return;
    const filtroTipo = document.getElementById('alerta_filtro_tipo')?.value || 'todas'; const filtroCat = document.getElementById('alerta_filtro_cat')?.value || 'Todas'; const ahora = Date.now(); let alertas = [];
    window.inventarioLocal.forEach(p => {
        if(filtroCat !== "Todas" && p.categoria !== filtroCat) return;
        const min = p.min_stk || 0; let msRestantesCad = p.caducidad ? (new Date(p.caducidad) - ahora) : null; let diasCad = msRestantesCad ? Math.ceil(msRestantesCad / 86400000) : null;
        let esBajoStock = p.stock <= min; let esPorCaducar = diasCad !== null && diasCad <= 30;
        if(!esBajoStock && !esPorCaducar) return; if(filtroTipo === 'stock' && !esBajoStock) return; if(filtroTipo === 'caducidad' && !esPorCaducar) return;
        let gananciaNum = (p.precio || 0) - (p.costo || 0); let estadoStr = [];
    ...
    let h = "";
    alertas.forEach(p => {
        const s = p.stock % 1 !== 0 ? p.stock.toFixed(3) : p.stock; let ganStr = p.gananciaNum > 0 ? `<span class="text-green-600 font-bold">+$${p.gananciaNum.toFixed(2)}</span>` : `<span class="text-gray-400">N/A</span>`; let provStr = p.proveedor ? `<span class="text-[10px] text-teal-600 font-bold block mt-1">🚚 ${p.proveedor}</span>` : '';
        h += `<tr class="border-b bg-white hover:bg-red-50 transition"><td class="p-3"><span class="font-mono text-gray-500 text-xs block">${p.sku}</span><span class="font-bold">${p.nombre}</span><span class="text-xs text-gray-400 block">${p.categoria||''}</span>${provStr}</td><td class="p-3 text-center"><span class="font-black text-lg text-red-600">${s}</span> <span class="text-gray-400 text-xs block mt-1">Límite: ${p.min_stk||0}</span></td><td class="p-3">${p.estadoHTML}</td><td class="p-3 text-right bg-green-50/50">${ganStr}</td></tr>`;
    });
    tb.innerHTML = h || "<tr><td colspan='4' class='p-8 text-center font-bold text-green-600'>✅ Todo en orden. Sin alertas actuales.</td></tr>";
};

window.abrirPestañaOrden = () => {
    if (window.ordenCompraActual.length === 0) {
        ...
        window.ordenCompraActual = faltantes.map(p => { let sugerido = (parseFloat(p.max_stk) || 0) > 0 ? (parseFloat(p.max_stk) - parseFloat(p.stock)) : 1; if (sugerido < 0) sugerido = 0; return { ...p, pedir: sugerido }; });
    }
    window.cambiarPestaña('tab-orden'); window.renderTabOrdenCompra();
};
...
window.renderTabOrdenCompra = () => {
    const contenedor = document.getElementById('contenedorTarjetasOrden'); if(!contenedor) return;
    let h = ""; let totalCosto = 0;
    if (window.ordenCompraActual.length === 0) { contenedor.innerHTML = `<div class="bg-white p-8 rounded-2xl text-center text-gray-400 font-bold border-2 border-dashed">No hay productos en la lista</div>`; return; }
    window.ordenCompraActual.forEach((p, idx) => {
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border-2 border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex-1 w-full">
                ...
                <div class="flex gap-4 mt-1 text-sm text-gray-500 font-bold"><span>Stock actual: <strong class="text-red-500">${stockActualStr}</strong></span><span>Costo: <strong>$${(parseFloat(p.costo)||0).toFixed(2)}</strong></span></div>
            </div>
            <div class="flex items-center justify-between w-full sm:w-auto gap-4 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                <div class="flex items-center gap-2">
                    <span class="text-xs font-bold text-gray-400 uppercase">Pedir:</span>
                    <input type="number" step="${p.es_granel ? '0.01' : '1'}" min="0" value="${p.pedir}" onchange="window.actualizarCantOrden(${idx}, this.value)" class="w-20 p-2 border-2 border-emerald-400 rounded-xl text-center font-black focus:ring-2 focus:ring-emerald-500">
                    <span class="text-xs font-bold text-gray-500">${p.es_granel ? 'L/Kg' : 'Pzas'}</span>
                </div>
                <div class="flex items-center gap-4">
                    <div class="text-right min-w-[80px]">
                        <span class="text-[10px] block text-gray-400 font-bold uppercase">Subtotal</span>
                        <span class="text-lg font-black text-orange-600">$${subCosto.toFixed(2)}</span>
                    </div>
                    <button onclick="window.eliminarDeOrden(${idx})" class="text-red-500 hover:bg-red-50 p-2 rounded-xl transition-colors" title="Eliminar de la lista">🗑️</button>
                </div>
            </div>
        </div>`;
    });
...
window.actualizarCantOrden = (idx, val) => {
    ...
    window.ordenCompraActual[idx].pedir = nuevo; window.renderTabOrdenCompra();
};
...
window.enviarOrdenWhatsApp = () => {
    ...
    if(filterPedir.length === 0) return alert("No hay productos con cantidad a pedir. Escribe artículos a pedir en las cajas de texto.");
    let textoMensaje = `📋 *NUEVA ORDEN DE COMPRA*\n🏢 *Negocio:* ${e}\n📍 *Sucursal:* ${z}\n\n*Productos solicitados:*\n`; let totalCosto = 0;
    filterPedir.forEach(p => { let subCosto = (parseFloat(p.costo) || 0) * p.pedir; totalCosto += subCosto; textoMensaje += `📦 ${p.pedir} x ${String(p.nombre).substring(0,25)}\n`; });
...
window.guardarProveedor = async () => {
    const nom = document.getElementById('prov_nombre').value.trim(); const tel = document.getElementById('prov_tel').value.trim(); const not = document.getElementById('prov_notas').value.trim(); const e = localStorage.getItem('empresaId');
    let dias = []; document.querySelectorAll('.prov_dia:checked').forEach(c => dias.push(c.value));
    if(!nom) return alert("Nombre obligatorio.");
    try { const id = nom.replace(/[^a-zA-Z0-9]/g, '').toUpperCase(); await setDoc(doc(db, `${e}_Proveedores`, id), { id, nombre: nom, telefono: tel, dias: dias, notas: not, timestamp: Date.now() }); window.mostrarNotificacion("✅ Proveedor guardado"); document.getElementById('prov_nombre').value = ''; document.getElementById('prov_tel').value = ''; document.getElementById('prov_notas').value = ''; document.querySelectorAll('.prov_dia').forEach(c => c.checked = false); window.cargarProveedores(); } catch(er) { alert("Error al guardar proveedor."); }
};
window.cargarProveedores = async () => {
    const e = localStorage.getItem('empresaId'); const tb = document.getElementById('tablaProveedoresBody'); tb.innerHTML = "<tr><td colspan='4' class='text-center p-8'>Cargando...</td></tr>"; let h = ""; let opts = '<option value="">Seleccionar Proveedor...</option>'; 
    try { const sn = await getDocs(collection(db, `${e}_Proveedores`)); sn.forEach(d => { const pr = d.data(); const diasStr = pr.dias.length > 0 ? pr.dias.join(', ') : 'Llamada previa'; h += `<tr class="border-b"><td class="p-3"><span class="font-bold text-teal-700 block">${pr.nombre}</span><span class="text-xs text-gray-500">📞 ${pr.telefono||'Sin Tel'}</span></td><td class="p-3 text-sm font-bold">${diasStr}</td><td class="p-3 text-xs text-gray-600 whitespace-pre-wrap">${pr.notas}</td><td class="p-3"><button onclick="eliminarProveedor('${pr.id}')" class="text-red-500 hover:bg-red-50 px-2 py-1 rounded transition">🗑️</button></td></tr>`; opts += `<option value="${pr.nombre}">${pr.nombre}</option>`; }); tb.innerHTML = h || "<tr><td colspan='4' class='text-center p-8'>Sin proveedores registrados</td></tr>"; const p_prov = document.getElementById('p_prov'); const edit_prov = document.getElementById('edit_prov'); if (p_prov) p_prov.innerHTML = opts; if (edit_prov) edit_prov.innerHTML = opts; } catch (er) { tb.innerHTML = "<tr><td colspan='4' class='text-center p-8 text-red-500'>Error</td></tr>"; }
};
window.eliminarProveedor = async (id) => { if(!confirm("¿Borrar este proveedor?")) return; const e = localStorage.getItem('empresaId'); try { await deleteDoc(doc(db, `${e}_Proveedores`, id)); window.cargarProveedores(); } catch(er) { alert("Error."); } };

// 10. EDICIÓN, REGISTRO Y CARGA MASIVA
window.guardarProducto = async () => {
    const sku = document.getElementById('p_sku').value.trim(); const nom = document.getElementById('p_nom').value.trim(); const cat = document.getElementById('p_cat').value.trim() || "General"; const prov = document.getElementById('p_prov')?.value || ""; const cos = parseFloat(document.getElementById('p_cos').value) || 0; const pre = parseFloat(document.getElementById('p_pre').value) || 0; const pMay = parseFloat(document.getElementById('p_mayoreo').value) || 0; const cMay = parseFloat(document.getElementById('p_cant_mayoreo').value) || 0; const stk = parseFloat(document.getElementById('p_stk').value) || 0; const minS = parseFloat(document.getElementById('p_min').value) || 0; const maxS = parseFloat(document.getElementById('p_max').value) || 0; const isGranel = document.getElementById('p_granel').checked; const cad = document.getElementById('p_caducidad').value || null; const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); const u = localStorage.getItem('currentUser'); const ts = Date.now(); const f = new Date(ts).toLocaleString('es-MX'); if(!sku || !nom) return alert("SKU y Nombre obligatorios.");
    try { const r = `${e}_Inventario_${z}`; const d = await getDoc(doc(db, r, sku)); let sF = stk; if (d.exists()) sF += d.data().stock; await setDoc(doc(db, r, sku), { sku, nombre: nom, categoria: cat, proveedor: prov, costo: cos, precio: pre, precio_mayoreo: pMay, cant_mayoreo: cMay, es_granel: isGranel, stock: sF, min_stk: minS, max_stk: maxS, caducidad: cad, zona: z, precio_promo: 0 }, { merge: true }); await addDoc(collection(db, `${e}_Historial_Ingresos`), { sku, nombre: nom, cantidad: stk, zona: z, usuario: u, fechaRegistro: f, timestamp: ts, tipoMovimiento: "ENTRADA" }); window.mostrarNotificacion("📦 Producto ingresado correctamente"); ['p_sku','p_nom','p_cat','p_cos','p_pre','p_mayoreo','p_cant_mayoreo','p_stk','p_min','p_max','p_caducidad'].forEach(id => document.getElementById(id).value = ''); if(document.getElementById('p_prov')) document.getElementById('p_prov').value = ''; document.getElementById('p_granel').checked = false; window.cargarInventarioGeneral(); } catch (err) { alert("Error al guardar producto."); }
};
window.abrirModalEdicion = (sku) => { 
    const p = window.inventarioLocal.find(x => x.sku === sku); if(!p) return;
    document.getElementById('edit_sku').value = p.sku; document.getElementById('edit_nom').value = p.nombre; document.getElementById('edit_cat').value = p.categoria || "General"; if(document.getElementById('edit_prov')) document.getElementById('edit_prov').value = p.proveedor || ""; document.getElementById('edit_cos').value = p.costo||0; document.getElementById('edit_pre').value = p.precio||0; document.getElementById('edit_promo').value = p.precio_promo || 0; document.getElementById('edit_mayoreo').value = p.precio_mayoreo || 0; document.getElementById('edit_cant_mayoreo').value = p.cant_mayoreo || 0; document.getElementById('edit_stk').value = p.stock||0; document.getElementById('edit_min').value = p.min_stk || 0; document.getElementById('edit_max').value = p.max_stk || 0; document.getElementById('edit_granel').checked = p.es_granel || false; document.getElementById('edit_caducidad').value = p.caducidad || ""; document.getElementById('modalEdicion').classList.remove('hidden'); 
};
window.cerrarModalEdicion = () => { document.getElementById('modalEdicion').classList.add('hidden'); };
window.guardarEdicionProducto = async () => {
    const sku = document.getElementById('edit_sku').value; const nom = document.getElementById('edit_nom').value.trim(); const cat = document.getElementById('edit_cat').value.trim() || "General"; const prov = document.getElementById('edit_prov')?.value || ""; const cos = parseFloat(document.getElementById('edit_cos').value) || 0; const pre = parseFloat(document.getElementById('edit_pre').value) || 0; const promo = parseFloat(document.getElementById('edit_promo').value) || 0; const pMay = parseFloat(document.getElementById('edit_mayoreo').value) || 0; const cMay = parseFloat(document.getElementById('edit_cant_mayoreo').value) || 0; const stk = parseFloat(document.getElementById('edit_stk').value) || 0; const minS = parseFloat(document.getElementById('edit_min').value) || 0; const maxS = parseFloat(document.getElementById('edit_max').value) || 0; const isGranel = document.getElementById('edit_granel').checked; const cad = document.getElementById('edit_caducidad').value || null; if(!nom) return alert("El nombre es obligatorio."); const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); const u = localStorage.getItem('currentUser'); const ts = Date.now(); const f = new Date(ts).toLocaleString('es-MX');
    try { let updates = { nombre: nom, categoria: cat, proveedor: prov, costo: cos, precio: pre, precio_promo: promo, precio_mayoreo: pMay, cant_mayoreo: cMay, es_granel: isGranel, stock: stk, min_stk: minS, max_stk: maxS, caducidad: cad }; await setDoc(doc(db, `${e}_Inventario_${z}`, sku), updates, { merge: true }); await addDoc(collection(db, `${e}_Historial_Ingresos`), { sku, nombre: nom, cantidad: stk, zona: z, usuario: u, fechaRegistro: f, timestamp: ts, tipoMovimiento: "AJUSTE" }); window.mostrarNotificacion("✅ Producto actualizado"); window.cerrarModalEdicion(); window.cargarInventarioGeneral(); } catch(err) { alert("Error al editar."); }
};
...
window.generarReporte = async () => {
    ...
    const tb = document.getElementById('tablaReportesBody'); tb.innerHTML = "<tr><td colspan='6' class='p-8 text-center'>Procesando ventas...</td></tr>"; 
    let totalDinero = 0; let cantVentas = 0; let cantTickets = 0; let h = ""; window.datosReporteCSV = [["Fecha", "Vendedor", "SKU", "Producto", "Categoria", "Cantidad", "Total_Venta"]]; 
    ...
    try {
        ...
        vSn.forEach(d => { const v = d.data(); if(v.timestamp >= tInicio && v.timestamp <= tFin) { if(fCat === 'Todas' || v.categoria === fCat) { ventasPeriodo.push(v); totalDinero += (v.precioVenta * v.cantidad); cantVentas += v.cantidad; } } });
        ...
        document.getElementById('rep_total_vendido').innerText = `$${totalDinero.toLocaleString('es-MX', {minimumFractionDigits: 2})}`; document.getElementById('rep_piezas_vendidas').innerText = cantVentas.toLocaleString(); document.getElementById('rep_ticket_promedio').innerText = `$${tkPromedio.toLocaleString('es-MX', {minimumFractionDigits: 2})}`;
...
window.descargarCSV = () => { 
    if(window.datosReporteCSV.length <= 1) return alert("Genera el reporte primero."); let csvContent = "data:text/csv;charset=utf-8,\uFEFF"; 
...
window.cargarKardex = async (tipo = 'entradas') => {
    window.kardexActual = tipo; document.getElementById('btn-kardex-entradas').className = tipo === 'entradas' ? 'px-4 py-2 font-bold text-green-600 border-b-4 border-green-600 transition' : 'px-4 py-2 font-bold text-gray-400 hover:text-red-500 border-b-4 border-transparent transition'; document.getElementById('btn-kardex-salidas').className = tipo === 'salidas' ? 'px-4 py-2 font-bold text-red-600 border-b-4 border-red-600 transition' : 'px-4 py-2 font-bold text-gray-400 hover:text-red-500 border-b-4 border-transparent transition';
    const z = document.getElementById('zonaSelect').value; const e = localStorage.getItem('empresaId'); const tb = document.getElementById('tablaKardexBody'); tb.innerHTML = "<tr><td colspan='6' class='p-8 text-center'>Consultando base de datos...</td></tr>"; let movs = [];
    try {
        if (tipo === 'entradas') { const qIngresos = query(collection(db, `${e}_Historial_Ingresos`), where("zona", "==", z)); const iSn = await getDocs(qIngresos); iSn.forEach(d => { movs.push({...d.data(), tipo: d.data().tipoMovimiento || 'ENTRADA'}); }); } 
        else { const qVentas = query(collection(db, `${e}_Historial_Ventas`), where("zona", "==", z)); const vSn = await getDocs(qVentas); vSn.forEach(d => { movs.push({...d.data(), tipo: 'SALIDA'}); }); }
        movs.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)); const movsRecientes = movs.slice(0, 100); let h = "";
        movsRecientes.forEach(m => {
            let bc = 'bg-gray-100 text-gray-700'; let ic = '⚙️'; if(m.tipo === 'ENTRADA') { bc = 'bg-green-100 text-green-700'; ic = '📦'; } if(m.tipo === 'SALIDA') { bc = 'bg-red-100 text-red-700'; ic = '🛍️'; } if(m.tipo === 'AJUSTE') { bc = 'bg-orange-100 text-orange-700'; ic = '✏️'; } if(m.tipo === 'ELIMINACIÓN') { bc = 'bg-red-800 text-white'; ic = '🗑️'; }
            const cV = m.cantidad % 1 !== 0 ? parseFloat(m.cantidad).toFixed(3) : m.cantidad; h += `<tr class="border-b"><td class="p-3 text-xs text-gray-500">${m.fechaRegistro}</td><td class="p-3"><span class="px-3 py-1 rounded text-xs font-bold ${bc}">${ic} ${m.tipo}</span></td><td class="p-3 font-mono text-xs">${m.sku}</td><td class="p-3 font-medium">${m.nombre}</td><td class="p-3 text-center font-bold">${cV}</td><td class="p-3 text-gray-600">${m.usuario}</td></tr>`;
...
window.abrirModalRegistroRapido = () => { const busquedaActual = document.getElementById('venta_busqueda').value; document.getElementById('rr_sku').value = busquedaActual.trim(); document.getElementById('rr_nombre').value = ''; document.getElementById('rr_costo').value = ''; document.getElementById('rr_precio').value = ''; document.getElementById('modal-registro-rapido').classList.remove('hidden'); if(busquedaActual !== "") { document.getElementById('rr_nombre').focus(); } else { document.getElementById('rr_sku').focus(); } };
...
window.guardarRegistroRapido = async () => {
    ...
    if (!sku || !nom || pre <= 0) { return alert('Por favor, ingresa SKU, Nombre y un Precio de Venta válido (mayor a 0).'); }
    ...
    try {
        ...
        window.inventarioLocal.push(nuevoProducto); window.cerrarModalRegistroRapido(); document.getElementById('venta_busqueda').value = ''; const productoParaCarrito = { ...nuevoProducto, stock: 9999 }; window.agregarAlCarrito(productoParaCarrito); window.mostrarNotificacion('✅ Producto express guardado y listo para cobro.');
    } catch (error) { alert('Hubo un error al registrar el producto en la base de datos.'); } finally { btn.innerHTML = btnOriginalText; btn.disabled = false; }
};